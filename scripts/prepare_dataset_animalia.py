...
assert len(train_list) == 1728
assert len(eval_list) == 1500

# now copy the selected classes to corresponding path
for cls in tqdm(train_list):
    files = list(cls.glob('*'))
    # indx_list = list(np.random.permutation(len(files))[:15])
    for indx in range(len(files)):
        f = files[indx]
        pastedir = Path(DATA_PATH + '/meta_dsets/animalia/images_background') / f.parent.name
...
for cls in tqdm(eval_list):
    ...
    for indx in range(len(files)):
        f = files[indx]
        pastedir = Path(DATA_PATH + '/meta_dsets/animalia/images_evaluation') / f.parent.name
        pastedir.mkdir(parents=True, exist_ok=True)
        shutil.copy(str(f), str(pastedir))

assert len(list(Path(DATA_PATH + '/meta_dsets/animalia/images_background').glob('*'))) == 1728
assert len(list(Path(DATA_PATH + '/meta_dsets/animalia/images_evaluation').glob('*'))) == 1500

path_dirs = [Path(DATA_PATH + '/meta_dsets/animalia/images_background'),
